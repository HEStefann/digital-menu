...
@section('mainContent')
    <form enctype="multipart/form-data"
        action="{{ route('product.store', ['menu' => $menu, 'category' => $category->id]) }}" method="post">
        @csrf
            ...
            <div class="col-span-4 space-y-6">
                ...
                <div class="flex justify-center items-center" x-data="imageModal()">
                    <label for="inputImage" id="labelInputImage"
                        class="z-10 px-[15px] opacity-[0.71] rounded-xl text-center text-[#4e4e4e] bg-[#FFFFFF] shadow-[0_2px_12px_0_rgba(32,32,32,18%)] text-[25px] absolute uppercase">add
                        photo +</label>
                ...
                <div>
                    <x-input-field icon="fa-solid fa-euro-sign" name="price" id="price" type="text"
                        inputmode="number" placeholder="Price" value="{{ old('price') }}" />
                    @error('price')
                        <div class="text-red-500 mt-1 mb-0 ml-9 text-start text-sm">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-span-4 space-y-6">
                <div>
                    <x-input-field icon="fa-regular fa-clock" name="preparation_time" id="prep_time" type="text"
                        inputmode="number" placeholder="Cooking Time (minutes)" value="{{ old('preparation_time') }}" />
                ...
                <div class="flex items-center justify-between">
                    <div class="w-[48%]">
                        <x-input-field name="weight" id="grams" type="text" placeholder="Grams" inputmode="numeric"
                            value="{{ old('weight') }}" />
                        @error('weight')
                            <div class="text-red-500 mt-1 mb-0 ml-9 text-start text-sm">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="w-[48%]">
                        <x-input-field name="callories" id="cal" type="text" placeholder="Callories"
                            inputmode="numeric" value="{{ old('callories') }}" />
                        @error('callories')
                            <div class="text-red-500 mt-1 mb-0 ml-9 text-start text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div>
                    <textarea
                        ...
                        name="ingredients" id="ingredients" rows="5" placeholder="Ingredients">{{ old('ingredients') }}</textarea>
                    @error('ingredients')
                        <div class="text-red-500 mt-1 mb-0 ml-9 text-start text-sm">{{ $message }}</div>
                    @enderror
                </div>
                        ...
                        @foreach ($allergens as $allergen)
                            <div class="flex items-center gap-2">
                                <input class="border-0 shadow-[0_2px_12px_0_rgb(32,32,32,18%)] rounded-md focus:ring-0"
                                    type="checkbox" id="{{ $allergen->id }}" name="allergens[]"
                                    value="{{ $allergen->id }}">
                ...
                <div class="flex items-center justify-end">
                    <button type="submit"
                        class="bg-white px-5 py-2 flex items-center justify-center text-[#707070] rounded-xl shadow-[0_2px_12px_0_rgb(32,32,32,18%)] hover:bg-[#F5F5F5] cursor-pointer">ADD
                        PRODUCT</button>
                </div>
            </div>
        </div>
